...
def roundupTime(dtarray=None, roundTo=60*60):
	...
	Args: dtarray : datetime.datetime object.
		roundTo : Closest number of seconds to round to, default 60 minutes.
		...
		for j in range(a):
			for i in range(len(dates)):
				...
				if(time_stop[j] == dates[i]):
					j_s = i
			total_cost.append(np.sum(np.asarray(price[i_s:(j_s-1)]))*charging_rate*eff)			# in dollars
			...
			for i in range(len(dates)):
				if(dates[i] == time):
					if((price[i] >= set_SP) and (battery_sold[j] <= battery_left_to_sell[j]) and (time < stop)):
						battery_sold[j] += charging_rate	#hourly
						money_earned += charging_rate*eff*price[i]	#hourly
...
for i in range(x):
	...
	grid = plt.GridSpec(2, 3, width_ratios = [3,1, 0.1], height_ratios=[1,3], hspace=0.05, wspace=0.05)
	main_ax = fig.add_subplot(grid[1,0])
	y_hist = fig.add_subplot(grid[1,1], xticklabels=[], yticklabels=[])#,sharey=main_ax)
	x_hist = fig.add_subplot(grid[0,0], yticklabels=[], xticklabels=[])# ,sharex=main_ax)
	cax = fig.add_subplot(grid[1,2])

	# scatter points on the main axes
	a = main_ax.hexbin(commute_time, time_depart, C = Annual_savings[i] , cmap = cmap, gridsize = 50, vmin = low, vmax = high)
	main_ax.set_xlabel('')

	# histogram on the attached axes
	x_hist.hist(commute_time, 50, histtype='stepfilled',
				orientation='vertical', color = '#ffa07a')
	#x_hist.invert_yaxis()

	y_hist.hist(time_depart, 50, histtype='stepfilled',
