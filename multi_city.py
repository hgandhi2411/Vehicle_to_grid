...
SP = [0.1] #np.arange(0, 0.31, 0.01)		#$/kWh
x = len(SP)
	...
	if(state == 'charging'):
		...
		for j in range(a):
			...
			for i in range(len(dates)):
				if(dates[i] == time):
					if((price[i] >= set_SP) and (battery_charged[j] <= battery*DoD) and (time < stop)):
				...
				if(battery_charged[j] < battery*DoD):
					cost += charging_rate * eff * (battery*DoD - battery_charged[j])/60 * price[i]
			total_cost[j] = cost
...
for s in state_list:
	...
	#LBMP Data
	data = pd.read_csv(prefix + lbmp_file, usecols = [0,1,2,3,4])
	...
	while(count <= working_days):
		...
		if(day.weekday()<5):
			...
			for i in range(x):
				...
				cost_charging, battery_charged = cost_calc('charging', date_set, price_set, time_charging_starts, time_stop = time_charging_stops) + battery_used[i] * bat_degradation * lcos * eff
			...
			cost_commute, battery_charged_commute = cost_calc('charging', date_set, price_set, time_charging_starts, time_stop = charge_commute_stop) + battery_used_for_travel * bat_degradation * lcos * eff
	...
	for i in range(x):
		...
		files.append(prefix + 'Results/' + s + '/Distance{}.svg'.format(SP[i]))
