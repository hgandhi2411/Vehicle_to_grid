...
path = 'C:/Users/hetag/Desktop/Vehicle_to_grid/Results/{}/'.format(dt.date.today())
if not os.path.exists(path):
    os.makedirs(path)
for i in range(x):
	files1.append('Savings{}.svg'.format(SP[i]))
	files.append('battery{}.svg'.format(SP[i]))
	files2.append('Time{}.svg'.format(SP[i]))
	files3.append('bat_dist{}.svg'.format(SP[i]))
...
for i in range(x):
	...
	# distance jointplot
	g = (sns.jointplot(battery, Annual_savings[i], kind = 'hex', gridsize = 15, marginal_kws={'bins': 15})).set_axis_labels("Commute distance", "Annual Savings")
	...
	# scatter points on the main axes
	a = main_ax.hexbin(battery, commute_dist, C = Annual_savings[i] , cmap = cmap, gridsize = 15, vmin = low, vmax = high)
	main_ax.set_xlabel('')
